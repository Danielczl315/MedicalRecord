...
<%@page import="edu.ncsu.csc.itrust.action.AddObstetricCareRecordAction"%>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        // Add Row CB
        $(".add-new").click(function(){
            var index = $("#childbirthVisitForm table tbody tr:last-child").index();
            var row = '<tr>' +
                '<td class="deliveryType"><select class="form-control" name="deliveryType" > ' +
                    <%
                    for(DeliveryType types : DeliveryType.values()){
                   ...
                   %>
                    ...
                    %>
                '</select></td>' +
                '<td class="hasDelivered"><input type="text" class="form-control" name="Delivered"></td>' +
                '<td class="pd"><input type="text" class="form-control" name="pd"></td>' +
                '<td class="nod"><input type="text" class="form-control" name="nod"></td>' +
                '<td class="ead"><input type="text" class="form-control" name="ead"></td>' +
                '<td class="msd"><input type="number" class="form-control" name="msd"></td>' +
                '<td class="rgd"><input type="number" class="form-control" name="rgd"></td>' +
                 + actions + '</td>' +
                '</tr>';
            $("#childbirthVisitForm table").append(row);
            $("#childbirthVisitForm table tbody tr").eq(index + 1).find(".save, .edit").toggle();
            $('[data-toggle="tooltip"]').tooltip();
        });
        // Save Row OB
      ...
      //Save Row CB
        $('#childbirthVisitForm').on("click", ".save", function(){
            var empty = false;
            var input = $(this).parents("tr").find('input');
            input.each(function(){
                if(!$(this).val()){
            ...
            if(!empty) {
                ...
                var prefDeliveryMethod = $(this).parents("tr").find(".prefDeliveryMethod").first();
                var hasDelivered = $(this).parents("tr").find(".hasDelivered").first();
                var pd = $(this).parents("tr").find(".pd").first();
                var nod = $(this).parents("tr").find(".nod").first();
                var ead = $(this).parents("tr").find(".ead").first();
                var msd = $(this).parents("tr").find(".msd").first();
                var rgd = $(this).parents("tr").find(".rgd").first();

                var obstetricRecordID = $(this).parents("tr").find(".obstetricsInitRecordID").first();
                var apptID = $(this).parents("tr").find(".apptID").first();
                var visitID = $(this).parents("tr").find(".visitID").first();
                var recordToPost = {
                    "prefDeliveryMethod":$(prefDeliveryMethod).find('select').val(),
                    "hasDelivered":$(hasDelivered).find('input').val(),
                    "pd":$(pd).find('input').val(),
                    "nod":$(nod).find('input').val(),
                    "ead":$(ead).find('input').val(),
                    "msd":$(msd).find('input').val(),
                    "rgd":$(rgd).find('input').val(),
                    "obstetricRecordID":$(obstetricRecordID).val(),
                    "apptID":$(apptID).val(),
                    "visitID":$(apptID).val()
                };
                submitChildbirthVisit(recordToPost, function(data) {
                    var response = data;
                    prefDeliveryMethod.html(response.prefDeliveryMethod);
                    hasDelivered.html(response.hasDelivered);
                    pd.html(response.pd);
                    nod.html(response.nod);
                    ead.html(response.ead);
                    msd.html(response.msd);
                    rgd.html(response.rgd);
                    //changes because the recordID should be -1 in case of new and should be changed for future edits
                    obstetricRecordID.html(response.obstetricRecordID);
                    apptID.html(response.apptID);
                    visitID.html(response.visitID);
                });
                $(this).parents("tr").find(".save, .edit").toggle();
            }
        });
        ...
        // Edit Row CB
        $('#childbirthVisitForm').on("click", ".edit", function(){
            //HTML cells
            ...
            hasDelivered.html('<input type="text" class="form-control" name="hasDelivered" value="' + hasDelivered.text() + '">');
            pd.html('<input type="text" class="form-control" name="pd" value="' + pd.text() + '">');
            nod.html('<input type="text" class="form-control" name="nod" value="' + nod.text() + '">');
            ead.html('<input type="text" class="form-control" name="ead" value="' + ead.text() + '">');
            msd.html('<input type="text" class="form-control" name="msd" value="' + msd.text() + '">');
            rgd.html('<input type="text" class="form-control" name="rgd" value="' + rgd.text() + '">');
            var currentDeliveryType = prefDeliveryMethod.text();
            prefDeliveryMethod.html('<select class="form-control" name="prefDeliveryMethod" >' +
                <%
                for(DeliveryType types : DeliveryType.values()){
                    String type = types.getName();
               %>
    ...
    function submitObstetricRecord(successCallback) {
     ...
     function submitChildbirthVisit(record, successCallback) {
         $.ajax({
...
<div class="table-wrapper">
     <div class="table-title">
          ...
          <div class="row">
               <div class="col-sm-8"><h2>Childbirth Visit</h2></div>
               <div class="col-sm-4">
     ...
     <table class="table table-bordered">
          ...
          <tbody>
		  <%
		  	List<ChildBirthVisitBean> childBirthVisitRecords = cb_action.getChildBirthVisitRecordByInitID(id);
          
      		for(ChildBirthVisitBean record: childBirthVisitRecords) {
          	 long visitID = record.getVisitID();
          	 long obstetricsInitRecordID = record.getObstetricsInitRecordID();
          	 long apptID = record.getApptID();
          	 DeliveryType preferredDeliveryMethod = record.getPreferredDeliveryMethod();
          	 BooleanType hasDelivered = record.getHasDelivered();
          	 long pitocinDosage = record.getPitocinDosage();
          	 long nitrousOxideDosage = record.getNitrousOxideDosage();
          	 long epiduralAnaesthesiaDosage = record.getEpiduralAnaesthesiaDosage();
          	 long magnesiumSulfateDosage = record.getMagnesiumSulfateDosage();
          	 long rhImmuneGlobulinDosage = record.getRhImmuneGlobulinDosage();
          ...
          <tr>
              ...
              <input type="hidden" class="obstetricsInitRecordID" value="<%=obstetricsInitRecordID%>">
              <input type="hidden" class="apptID" value="<%=apptID%>">
              <input type="hidden" class="visitID" value="<%=visitID%>">
              <td>
                    <a class="save" title="Save" data-toggle="tooltip"><i class="fa fa-check"></i></a>
                    <a class="edit" title="Edit" data-toggle="tooltip"><i class="material-icons">&#xE254;</i></a>
               </td>
